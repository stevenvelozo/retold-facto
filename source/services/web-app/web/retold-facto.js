...
`}],Renderables:[{RenderableHash:"Facto-Full-Records-Content",TemplateHash:"Facto-Full-Records-Template",DestinationAddress:"#Facto-Full-Content-Container",RenderMethod:"replace"}]};class FactoFullRecordsView extends libPictView{constructor(pFable,pOptions,pServiceHash){super(pFable,pOptions,pServiceHash);}onAfterRender(pRenderable,pRenderDestinationAddress,pRecord,pContent){this.pict.providers.Facto.loadRecords(this.pict.AppData.Facto.RecordPage).then(()=>{this.refreshList();});return super.onAfterRender(pRenderable,pRenderDestinationAddress,pRecord,pContent);}refreshList(){let tmpContainer=document.getElementById('Facto-Full-Records-List');if(!tmpContainer)return;let tmpRecords=this.pict.AppData.Facto.Records;let tmpPageInfo=document.getElementById('Facto-Full-Records-PageInfo');if(tmpPageInfo)tmpPageInfo.textContent='Page '+((this.pict.AppData.Facto.RecordPage||0)+1);if(!tmpRecords||tmpRecords.length===0){tmpContainer.innerHTML='<div class="facto-empty">No records found. Ingest data via Source Research or the Ingest API.</div>';return;}let tmpHtml='<table><thead><tr><th>ID</th><th>Dataset</th><th>Source</th><th>GUID</th><th>Data</th><th></th></tr></thead><tbody>';for(let i=0;i<tmpRecords.length;i++){let tmpRec=tmpRecords[i];let tmpData='';try{tmpData=JSON.stringify(JSON.parse(tmpRec.Content||'{}'));}catch(e){tmpData=tmpRec.Content||'';}tmpHtml+='<tr>';tmpHtml+='<td>'+(tmpRec.IDRecord||'')+'</td>';tmpHtml+='<td>'+(tmpRec.IDDataset||'')+'</td>';tmpHtml+='<td>'+(tmpRec.IDSource||'')+'</td>';tmpHtml+='<td style="font-size:0.8em; color:var(--facto-text-tertiary);">'+(tmpRec.GUIDRecord||'').substring(0,8)+'...</td>';tmpHtml+='<td class="facto-record-data">'+tmpData+'</td>';tmpHtml+='<td><button class="facto-btn facto-btn-secondary facto-btn-small" onclick="pict.PictApplication.navigateTo(\'/Record/'+tmpRec.IDRecord+'\')">View</button></td>';tmpHtml+='</tr>';}tmpHtml+='</tbody></table>';tmpContainer.innerHTML=tmpHtml;}prevPage(){if(this.pict.AppData.Facto.RecordPage>0){this.pict.AppData.Facto.RecordPage--;this.pict.providers.Facto.loadRecords(this.pict.AppData.Facto.RecordPage).then(()=>{this.refreshList();});}}nextPage(){this.pict.AppData.Facto.RecordPage++;this.pict.providers.Facto.loadRecords(this.pict.AppData.Facto.RecordPage).then(()=>{this.refreshList();});}}module.exports=FactoFullRecordsView;module.exports.default_configuration=_ViewConfiguration;},{"pict-view":64}],77:[function(require,module,exports){const libPictView=require('pict-view');const _ViewConfiguration={ViewIdentifier:"Facto-Full-Scanner",DefaultRenderable:"Facto-Full-Scanner-Content",DefaultDestinationAddress:"#Facto-Full-Content-Container",AutoRender:false,CSS:/*css*/`
	...
	`,Templates:[{Hash:"Facto-Full-Scanner-Template",Template:/*html*/`
<div class="facto-content">
	<div class="facto-content-header">
...
`}],Renderables:[{RenderableHash:"Facto-Full-Scanner-Content",TemplateHash:"Facto-Full-Scanner-Template",DestinationAddress:"#Facto-Full-Content-Container",RenderMethod:"replace"}]};class FactoFullScannerView extends libPictView{constructor(pFable,pOptions,pServiceHash){super(pFable,pOptions,pServiceHash);}onAfterRender(pRenderable,pRenderDestinationAddress,pRecord,pContent){this.loadScannerState();return super.onAfterRender(pRenderable,pRenderDestinationAddress,pRecord,pContent);}setStatus(pMessage,pType){let tmpEl=document.getElementById('Facto-Full-Scanner-Status');if(!tmpEl)return;tmpEl.className='facto-status facto-status-'+(pType||'info');tmpEl.textContent=pMessage;tmpEl.style.display='block';tmpEl.scrollIntoView({behavior:'smooth',block:'nearest'});}loadScannerState(){Promise.all([this.pict.providers.Facto.loadScannerPaths(),this.pict.providers.Facto.loadScannerDatasets()]).then(()=>{this.refreshPathsList();this.refreshDatasetsList();}).catch(pError=>{this.setStatus('Error loading scanner state: '+pError.message,'error');});}refreshPathsList(){let tmpContainer=document.getElementById('Facto-Full-Scanner-PathsList');if(!tmpContainer)return;let tmpPaths=this.pict.AppData.Facto.ScannerPaths||[];if(tmpPaths.length===0){tmpContainer.innerHTML='<div class="facto-empty">No scan paths configured. Add a folder path to discover datasets.</div>';return;}let tmpHtml='<table><thead><tr><th>Path</th><th>Datasets</th><th>Last Scanned</th><th>Actions</th></tr></thead><tbody>';for(let i=0;i<tmpPaths.length;i++){let tmpPath=tmpPaths[i];let tmpLastScanned=tmpPath.LastScannedAt?new Date(tmpPath.LastScannedAt).toLocaleString():'Never';tmpHtml+='<tr>';tmpHtml+='<td style="font-family:\'SF Mono\', Consolas, monospace; font-size:0.85em;">'+this.escapeHtml(tmpPath.Path)+'</td>';tmpHtml+='<td>'+(tmpPath.DatasetCount||0)+'</td>';tmpHtml+='<td>'+tmpLastScanned+'</td>';tmpHtml+='<td><button class="facto-btn facto-btn-danger facto-btn-small" onclick="pict.views[\'Facto-Full-Scanner\'].removePath(\''+this.escapeAttr(tmpPath.Path)+'\')">Remove</button></td>';tmpHtml+='</tr>';}tmpHtml+='</tbody></table>';tmpContainer.innerHTML=tmpHtml;}refreshDatasetsList(){let tmpContainer=document.getElementById('Facto-Full-Scanner-DatasetsList');if(!tmpContainer)return;let tmpDatasets=this.pict.AppData.Facto.ScannerDatasets||[];if(tmpDatasets.length===0){tmpContainer.innerHTML='<div class="facto-empty">No datasets discovered yet. Add a scan path containing dataset folders with README.md files.</div>';this.updateSummary(0,0,0,0);return;}// Compute summary stats
...
let tmpSearchEl=document.getElementById('Facto-Full-Scanner-Search');let tmpSearchTerm=tmpSearchEl?tmpSearchEl.value.toLowerCase():'';let tmpStatusFilter=document.getElementById('Facto-Full-Scanner-StatusFilter');let tmpStatusValue=tmpStatusFilter?tmpStatusFilter.value:'';let tmpFiltered=tmpDatasets;if(tmpSearchTerm){tmpFiltered=tmpFiltered.filter(pDS=>{let tmpText=((pDS.FolderName||'')+' '+(pDS.Title||'')+' '+(pDS.Provider||'')).toLowerCase();return tmpText.indexOf(tmpSearchTerm)>-1;});}if(tmpStatusValue){tmpFiltered=tmpFiltered.filter(pDS=>{return pDS.Status===tmpStatusValue;});}let tmpHtml='<table><thead><tr>';tmpHtml+='<th><input type="checkbox" id="Facto-Full-Scanner-SelectAll" onclick="pict.views[\'Facto-Full-Scanner\'].toggleSelectAll(this.checked)"></th>';tmpHtml+='<th>Name</th><th>Title</th><th>Provider</th><th>Format</th><th>Data</th><th>Status</th><th>Actions</th>';tmpHtml+='</tr></thead><tbody>';for(let i=0;i<tmpFiltered.length;i++){let tmpDS=tmpFiltered[i];let tmpStatusBadge=this.getStatusBadge(tmpDS.Status);let tmpDataInfo=tmpDS.HasData?tmpDS.DataFileCount+' file'+(tmpDS.DataFileCount!==1?'s':'')+' ('+(tmpDS.TotalDataSizeFormatted||'0 B')+')':'<span style="color:var(--facto-text-danger, #dc3545);">No data</span>';let tmpFormat=tmpDS.DataFormat?tmpDS.DataFormat.Format||'unknown':'unknown';let tmpEscFolder=this.escapeAttr(tmpDS.FolderName);tmpHtml+='<tr id="facto-scanner-row-'+tmpEscFolder+'">';tmpHtml+='<td><input type="checkbox" class="facto-scanner-checkbox" data-folder="'+tmpEscFolder+'"></td>';tmpHtml+='<td style="font-family:\'SF Mono\', Consolas, monospace; font-size:0.85em;">'+this.escapeHtml(tmpDS.FolderName)+'</td>';tmpHtml+='<td>'+this.escapeHtml((tmpDS.Title||'').substring(0,50))+'</td>';tmpHtml+='<td>'+this.escapeHtml((tmpDS.Provider||'').substring(0,30))+'</td>';tmpHtml+='<td><span class="facto-badge facto-badge-primary">'+tmpFormat+'</span></td>';tmpHtml+='<td>'+tmpDataInfo+'</td>';tmpHtml+='<td>'+tmpStatusBadge+'</td>';tmpHtml+='<td>';if(tmpDS.Status==='Discovered'){tmpHtml+='<button class="facto-btn facto-btn-success facto-btn-small" onclick="pict.views[\'Facto-Full-Scanner\'].provisionOne(\''+tmpEscFolder+'\')">Provision</button> ';}if((tmpDS.Status==='Provisioned'||tmpDS.Status==='Discovered')&&tmpDS.HasData){tmpHtml+='<button class="facto-btn facto-btn-primary facto-btn-small" onclick="pict.views[\'Facto-Full-Scanner\'].ingestOne(\''+tmpEscFolder+'\')">Ingest</button> ';}tmpHtml+='<button class="facto-btn facto-btn-secondary facto-btn-small" onclick="pict.views[\'Facto-Full-Scanner\'].viewDetail(\''+tmpEscFolder+'\')">Detail</button>';tmpHtml+='</td>';tmpHtml+='</tr>';}tmpHtml+='</tbody></table>';tmpHtml+='<div style="color:var(--facto-text-secondary); font-size:0.85em; margin-top:0.5em;">Showing '+tmpFiltered.length+' of '+tmpDatasets.length+' dataset(s)</div>';tmpContainer.innerHTML=tmpHtml;}updateSummary(pTotal,pDiscovered,pProvisioned,pWithData){let tmpEl=document.getElementById('Facto-Full-Scanner-Summary');if(!tmpEl)return;tmpEl.innerHTML='<div class="facto-scanner-summary-stat"><strong>'+pTotal+'</strong> discovered</div>'+'<div class="facto-scanner-summary-stat"><strong>'+pProvisioned+'</strong> provisioned</div>'+'<div class="facto-scanner-summary-stat"><strong>'+pWithData+'</strong> with data</div>';}getStatusBadge(pStatus){if(pStatus==='Discovered')return'<span class="facto-badge facto-badge-info">Discovered</span>';if(pStatus==='Provisioned')return'<span class="facto-badge facto-badge-success">Provisioned</span>';if(pStatus==='Ingested')return'<span class="facto-badge facto-badge-warning">Ingested</span>';if(pStatus==='Error')return'<span class="facto-badge facto-badge-danger">Error</span>';return'<span class="facto-badge facto-badge-muted">'+(pStatus||'')+'</span>';}escapeHtml(pStr){if(!pStr)return'';return pStr.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}escapeAttr(pStr){if(!pStr)return'';return pStr.replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/"/g,'&quot;');}formatSize(pBytes){if(pBytes===0)return'0 B';let tmpUnits=['B','KB','MB','GB'];let tmpI=Math.floor(Math.log(pBytes)/Math.log(1024));if(tmpI>=tmpUnits.length)tmpI=tmpUnits.length-1;return(pBytes/Math.pow(1024,tmpI)).toFixed(tmpI>0?1:0)+' '+tmpUnits[tmpI];}// ================================================================
// Actions
// ================================================================
addPath(){let tmpPathInput=document.getElementById('Facto-Full-Scanner-PathInput');let tmpPath=tmpPathInput?tmpPathInput.value.trim():'';if(!tmpPath){this.setStatus('Enter a folder path to scan','warn');return;}this.setStatus('Scanning '+tmpPath+'...','info');this.pict.providers.Facto.addScannerPath(tmpPath).then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Error: '+pResponse.Error,'error');return;}let tmpResult=pResponse.ScanResult||{};this.setStatus('Scanned! Found '+(tmpResult.DatasetsFound||0)+' dataset(s) in '+(tmpResult.FoldersScanned||0)+' folder(s)','ok');if(tmpPathInput)tmpPathInput.value='';this.loadScannerState();}).catch(pError=>{this.setStatus('Error: '+pError.message,'error');});}removePath(pPath){if(!confirm('Remove scan path and its discovered datasets?\n\n'+pPath))return;this.pict.providers.Facto.removeScannerPath(pPath).then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Error: '+pResponse.Error,'error');return;}this.setStatus('Path removed','ok');this.loadScannerState();}).catch(pError=>{this.setStatus('Error: '+pError.message,'error');});}rescanAll(){this.setStatus('Re-scanning all paths...','info');this.pict.providers.Facto.rescanPaths().then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Error: '+pResponse.Error,'error');return;}this.setStatus('Re-scan complete','ok');this.loadScannerState();}).catch(pError=>{this.setStatus('Error: '+pError.message,'error');});}filterDatasets(){this.refreshDatasetsList();}toggleSelectAll(pChecked){let tmpCheckboxes=document.querySelectorAll('.facto-scanner-checkbox');for(let i=0;i<tmpCheckboxes.length;i++){tmpCheckboxes[i].checked=pChecked;}}getSelectedFolderNames(){let tmpCheckboxes=document.querySelectorAll('.facto-scanner-checkbox:checked');let tmpNames=[];for(let i=0;i<tmpCheckboxes.length;i++){tmpNames.push(tmpCheckboxes[i].getAttribute('data-folder'));}return tmpNames;}provisionOne(pFolderName){this.setStatus('Provisioning '+pFolderName+'...','info');this.pict.providers.Facto.provisionScannerDataset(pFolderName).then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Error: '+pResponse.Error,'error');return;}this.setStatus('Provisioned '+pFolderName+' (Source #'+(pResponse.Source?pResponse.Source.IDSource:'?')+', Dataset #'+(pResponse.Dataset?pResponse.Dataset.IDDataset:'?')+')','ok');this.loadScannerState();}).catch(pError=>{this.setStatus('Error: '+pError.message,'error');});}ingestOne(pFolderName){this.setStatus('Ingesting '+pFolderName+'...','info');this.pict.providers.Facto.ingestScannerDataset(pFolderName).then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Ingest error: '+pResponse.Error,'error');return;}let tmpMsg='Ingested '+pFolderName;if(pResponse.RecordsIngested!==undefined){tmpMsg+=' — '+pResponse.RecordsIngested+' records';}if(pResponse.File){tmpMsg+=' from '+pResponse.File;}this.setStatus(tmpMsg,'ok');this.loadScannerState();}).catch(pError=>{this.setStatus('Ingest error: '+pError.message,'error');});}provisionSelected(){let tmpSelected=this.getSelectedFolderNames();if(tmpSelected.length===0){this.setStatus('Select datasets to provision using the checkboxes','warn');return;}if(!confirm('Provision '+tmpSelected.length+' selected dataset(s)?'))return;this.provisionBatch(tmpSelected,0,0,0);}provisionAll(){if(!confirm('Provision ALL discovered datasets?'))return;this.setStatus('Provisioning all datasets...','info');this.pict.providers.Facto.provisionAllScannerDatasets().then(pResponse=>{if(pResponse&&pResponse.Error){this.setStatus('Error: '+pResponse.Error,'error');return;}this.setStatus('Provisioned '+pResponse.Provisioned+' of '+pResponse.Total+' ('+pResponse.Errors+' error(s))','ok');this.loadScannerState();}).catch(pError=>{this.setStatus('Error: '+pError.message,'error');});}provisionBatch(pFolderNames,pIndex,pSuccessCount,pErrorCount){if(pIndex>=pFolderNames.length){this.setStatus('Provisioned '+pSuccessCount+' of '+pFolderNames.length+' ('+pErrorCount+' error(s))','ok');this.loadScannerState();return;}let tmpName=pFolderNames[pIndex];this.setStatus('Provisioning '+(pIndex+1)+'/'+pFolderNames.length+': '+tmpName+'...','info');this.pict.providers.Facto.provisionScannerDataset(tmpName).then(pResponse=>{if(pResponse&&pResponse.Error){this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount,pErrorCount+1);}else{this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount+1,pErrorCount);}}).catch(()=>{this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount,pErrorCount+1);});}viewDetail(pFolderName){let tmpDetailRowId='facto-scanner-detail-'+pFolderName;let tmpExisting=document.getElementById(tmpDetailRowId);// Toggle: if already open, close it
if(tmpExisting){tmpExisting.remove();return;}// Close any other open detail row
let tmpOldDetails=document.querySelectorAll('.facto-scanner-detail-row');for(let i=0;i<tmpOldDetails.length;i++){tmpOldDetails[i].remove();}// Find the clicked row and insert a detail row right after it
let tmpRow=document.getElementById('facto-scanner-row-'+this.escapeAttr(pFolderName));if(!tmpRow)return;let tmpColCount=tmpRow.children.length;let tmpDetailRow=document.createElement('tr');tmpDetailRow.id=tmpDetailRowId;tmpDetailRow.className='facto-scanner-detail-row';tmpDetailRow.innerHTML='<td colspan="'+tmpColCount+'"><div class="facto-scanner-detail-panel" style="margin:0;"><span style="color:var(--facto-text-secondary);">Loading details for '+this.escapeHtml(pFolderName)+'...</span></div></td>';tmpRow.after(tmpDetailRow);this.pict.providers.Facto.loadScannerDatasetDetail(pFolderName).then(pDS=>{// Row might have been removed while loading
let tmpTarget=document.getElementById(tmpDetailRowId);if(!tmpTarget)return;if(pDS&&pDS.Error){tmpTarget.innerHTML='<td colspan="'+tmpColCount+'"><div class="facto-status facto-status-error" style="margin:0;">'+pDS.Error+'</div></td>';return;}let tmpHtml='<td colspan="'+tmpColCount+'"><div class="facto-scanner-detail-panel" style="margin:0;">';tmpHtml+='<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1em;">';tmpHtml+='<h3 style="margin:0;">'+this.escapeHtml(pDS.Title||pDS.FolderName)+'</h3>';tmpHtml+='<button class="facto-btn facto-btn-secondary" onclick="pict.views[\'Facto-Full-Scanner\'].viewDetail(\''+this.escapeAttr(pFolderName)+'\')">Close</button>';tmpHtml+='</div>';// Metadata grid
tmpHtml+='<div class="facto-scanner-detail-grid">';tmpHtml+='<div><strong>Folder:</strong> <span style="font-family:\'SF Mono\', Consolas, monospace;">'+this.escapeHtml(pDS.FolderName)+'</span></div>';tmpHtml+='<div><strong>Status:</strong> '+this.getStatusBadge(pDS.Status)+'</div>';tmpHtml+='<div><strong>Provider:</strong> '+this.escapeHtml(pDS.Provider||'Unknown')+'</div>';tmpHtml+='<div><strong>License:</strong> '+this.escapeHtml(pDS.License||'Unknown')+'</div>';if(pDS.SourceURL){tmpHtml+='<div><strong>Source:</strong> <a href="'+this.escapeHtml(pDS.SourceURL)+'" target="_blank" style="color:var(--facto-brand);">'+this.escapeHtml(pDS.SourceURL.substring(0,60))+'</a></div>';}if(pDS.UpdateFrequency){tmpHtml+='<div><strong>Update Frequency:</strong> '+this.escapeHtml(pDS.UpdateFrequency.substring(0,100))+'</div>';}if(pDS.RecordCount){tmpHtml+='<div><strong>Record Count:</strong> '+this.escapeHtml(pDS.RecordCount.substring(0,100))+'</div>';}if(pDS.IDSource){tmpHtml+='<div><strong>Source ID:</strong> <a href="#/Source/'+pDS.IDSource+'" style="color:var(--facto-brand);">#'+pDS.IDSource+'</a></div>';}if(pDS.IDDataset){tmpHtml+='<div><strong>Dataset ID:</strong> #'+pDS.IDDataset+'</div>';}tmpHtml+='</div>';// Description
...
// Actions
// ================================================================
addPath(){let tmpPathInput=document.getElementById('facto-scanner-path-input');let tmpPath=tmpPathInput?tmpPathInput.value.trim():'';if(!tmpPath){this.pict.providers.Facto.setStatus('facto-scanner-status','Enter a folder path to scan','warn');return;}this.pict.providers.Facto.setStatus('facto-scanner-status','Scanning '+tmpPath+'...','info');this.pict.providers.Facto.addScannerPath(tmpPath).then(pResponse=>{if(pResponse&&pResponse.Error){this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pResponse.Error,'error');return;}let tmpResult=pResponse.ScanResult||{};this.pict.providers.Facto.setStatus('facto-scanner-status','Scanned! Found '+(tmpResult.DatasetsFound||0)+' dataset(s) in '+(tmpResult.FoldersScanned||0)+' folder(s)','ok');if(tmpPathInput)tmpPathInput.value='';this.loadScannerState();}).catch(pError=>{this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pError.message,'error');});}removePath(pPath){if(!confirm('Remove scan path and its discovered datasets?\n\n'+pPath))return;this.pict.providers.Facto.removeScannerPath(pPath).then(pResponse=>{if(pResponse&&pResponse.Error){this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pResponse.Error,'error');return;}this.pict.providers.Facto.setStatus('facto-scanner-status','Path removed','ok');this.loadScannerState();}).catch(pError=>{this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pError.message,'error');});}rescanAll(){this.pict.providers.Facto.setStatus('facto-scanner-status','Re-scanning all paths...','info');this.pict.providers.Facto.rescanPaths().then(pResponse=>{if(pResponse&&pResponse.Error){this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pResponse.Error,'error');return;}this.pict.providers.Facto.setStatus('facto-scanner-status','Re-scan complete','ok');this.loadScannerState();}).catch(pError=>{this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pError.message,'error');});}filterDatasets(){this.refreshDatasetsList();}toggleSelectAll(pChecked){let tmpCheckboxes=document.querySelectorAll('.facto-scanner-checkbox');for(let i=0;i<tmpCheckboxes.length;i++){tmpCheckboxes[i].checked=pChecked;}}getSelectedFolderNames(){let tmpCheckboxes=document.querySelectorAll('.facto-scanner-checkbox:checked');let tmpNames=[];for(let i=0;i<tmpCheckboxes.length;i++){tmpNames.push(tmpCheckboxes[i].getAttribute('data-folder'));}return tmpNames;}provisionOne(pFolderName){this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioning '+pFolderName+'...','info');this.pict.providers.Facto.provisionScannerDataset(pFolderName).then(pResponse=>{if(pResponse&&pResponse.Error){this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pResponse.Error,'error');return;}this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioned '+pFolderName+' (Source #'+(pResponse.Source?pResponse.Source.IDSource:'?')+', Dataset #'+(pResponse.Dataset?pResponse.Dataset.IDDataset:'?')+')','ok');this.loadScannerState();// Refresh sources/datasets views
this.refreshRelatedViews();}).catch(pError=>{this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pError.message,'error');});}provisionSelected(){let tmpSelected=this.getSelectedFolderNames();if(tmpSelected.length===0){this.pict.providers.Facto.setStatus('facto-scanner-status','Select datasets to provision using the checkboxes','warn');return;}if(!confirm('Provision '+tmpSelected.length+' selected dataset(s)?'))return;this.provisionBatch(tmpSelected,0,0,0);}provisionAll(){if(!confirm('Provision ALL discovered datasets?'))return;this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioning all datasets...','info');this.pict.providers.Facto.provisionAllScannerDatasets().then(pResponse=>{if(pResponse&&pResponse.Error){this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pResponse.Error,'error');return;}this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioned '+pResponse.Provisioned+' of '+pResponse.Total+' ('+pResponse.Errors+' error(s))','ok');this.loadScannerState();this.refreshRelatedViews();}).catch(pError=>{this.pict.providers.Facto.setStatus('facto-scanner-status','Error: '+pError.message,'error');});}provisionBatch(pFolderNames,pIndex,pSuccessCount,pErrorCount){if(pIndex>=pFolderNames.length){this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioned '+pSuccessCount+' of '+pFolderNames.length+' ('+pErrorCount+' error(s))','ok');this.loadScannerState();this.refreshRelatedViews();return;}let tmpName=pFolderNames[pIndex];this.pict.providers.Facto.setStatus('facto-scanner-status','Provisioning '+(pIndex+1)+'/'+pFolderNames.length+': '+tmpName+'...','info');this.pict.providers.Facto.provisionScannerDataset(tmpName).then(pResponse=>{if(pResponse&&pResponse.Error){this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount,pErrorCount+1);}else{this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount+1,pErrorCount);}}).catch(()=>{this.provisionBatch(pFolderNames,pIndex+1,pSuccessCount,pErrorCount+1);});}viewDetail(pFolderName){let tmpContainer=document.getElementById('facto-scanner-detail');if(!tmpContainer)return;tmpContainer.innerHTML='<p style="color:#888;">Loading details for '+this.escapeHtml(pFolderName)+'...</p>';this.pict.providers.Facto.loadScannerDatasetDetail(pFolderName).then(pDS=>{if(pDS&&pDS.Error){tmpContainer.innerHTML='<p style="color:#dc3545;">'+pDS.Error+'</p>';return;}let tmpHtml='<div style="border:1px solid #ddd; border-radius:6px; padding:16px; background:#fafafa;">';tmpHtml+='<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">';tmpHtml+='<h3 style="margin:0; font-size:1.1em; color:#333;">'+this.escapeHtml(pDS.Title||pDS.FolderName)+'</h3>';tmpHtml+='<button class="secondary" style="padding:4px 10px; font-size:0.82em;" onclick="document.getElementById(\'facto-scanner-detail\').innerHTML=\'\';">Close</button>';tmpHtml+='</div>';// Metadata grid
...
tmpHtml+='<div>';if(pDS.Status==='Discovered'){tmpHtml+='<button class="success" onclick="pict.views[\'Facto-Scanner\'].provisionOne(\''+this.escapeAttr(pDS.FolderName)+'\')">Provision</button>';}tmpHtml+='</div>';tmpHtml+='</div>';tmpContainer.innerHTML=tmpHtml;}).catch(pError=>{tmpContainer.innerHTML='<p style="color:#dc3545;">Error: '+pError.message+'</p>';});}formatSize(pBytes){if(pBytes===0)return'0 B';let tmpUnits=['B','KB','MB','GB','TB'];let tmpIndex=Math.floor(Math.log(pBytes)/Math.log(1024));return(pBytes/Math.pow(1024,tmpIndex)).toFixed(1)+' '+tmpUnits[tmpIndex];}refreshRelatedViews(){if(this.pict.views['Facto-Sources']){this.pict.providers.Facto.loadSources().then(()=>{this.pict.views['Facto-Sources'].refreshList();});}if(this.pict.views['Facto-Datasets']){this.pict.providers.Facto.loadDatasets().then(()=>{this.pict.views['Facto-Datasets'].refreshList();});}}}module.exports=FactoScannerView;module.exports.default_configuration={ViewIdentifier:'Facto-Scanner',DefaultRenderable:'Facto-Scanner',DefaultDestinationAddress:'#Facto-Section-Scanner',Templates:[{Hash:'Facto-Scanner',Template:/*html*/`
<div class="accordion-row">
